    ...
    for cab, info in remates_dict.items():
        ...
        html += f"""
                <tr>
                    ...
                    <td style="font-weight: 800; font-size: 12px;" title="{nombre_solo.upper()}">{nombre_solo.upper()}</td>
            ...
            for i in range(0, len(carreras_totales_visibles), chunk_size):
                ...
                for idx, c_nombre in enumerate(chunk):
                    ...
                    with cols_carreras[idx]:
                        if st.button(abreviatura, key=f"rem_btn_sel_{modo_actual_remate}_{i + idx}_{c_nombre}", use_container_width=True, type="primary" if es_activa else "secondary"):
