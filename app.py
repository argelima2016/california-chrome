...
st.markdown("""
    ...
    /* --- CABECERA MÓVIL MODERNA Y COMPACTA --- */
    .header-container {
...
# Se organizan los 4 botones principales en disposición vertical fluida y limpia
...
if menu_principal_opcion == "Remates":
    if not lista_carreras_disponibles:
        ...
        if not carreras_filtradas_visibles:
            ...
            # --- SELECCIONADOR DE CARRERAS EN LISTA VERTICAL ESTÉTICA (C1, C2, C3...) ---
            # En lugar de botones horizontales amontonados, se muestran de forma vertical limpia y uniforme
        ...
        if st.session_state.programa_pdf_bytes is not None:
            ...
            with col_ps2:
                if st.button("⚡ Procesar PDF Completo Organizado", use_container_width=True):
                    if procesar_date = procesar_texto_para_remates(st.session_state.texto_completo_pdf):
...
st.markdown("<hr style='margin: 0.5rem 0 1rem 0; border-color: #21262d;'>", unsafe_allow_html=True)
