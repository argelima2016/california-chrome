            ...
            if "carrera" in linea_lower or any(k in linea_lower for k in map_numeros.keys()):
                ...
                if match_digito and ("carrera" in linea_lower or len(linea_limpia) < 40):
                    ...
                    if carrera_actual_detectada not in detalles_temporal:
                        detalles_temporal[carrera_actual_detectada] = {"condicion": linea_limpia, "distancia": "Por definir", "hora": "Por definir"}
                    continue
                else:
...
# =========================================================================
    ...
    if tab_actual == "✍️ Banco":
        ...
        st.markdown("Pega aquí abajo el contenido copiado de la página web de inscritos (o la URL de una página web pública de texto):")
...
def extraer_texto_pdf_desde_bytes(bytes_pdf, nombre_archivo="Inscritos_Remota.pdf"):
    try:
        st.session_state.programa_pdf_bytes = bytes_pdf
        st.session_state.programa_pdf_nombre = nombre_archivo
        archivo_io = io.BytesIO(bytes_pdf)
        lector_pdf = PdfReader(archivo_io)
        texto_extraido = ""
...
# =========================================================================
    ...
    if tab_actual == "✍️ Banco":
        ...
        st.markdown("### 📄 Lector PDF e Importador por URL o Archivo")
        
        url_pdf = st.text_input("🔗 Pegar URL del PDF de Inscritos:", placeholder="https://ejemplo.com/programa.pdf", key="input_url_pdf")
        if st.button("🌐 Descargar y Procesar desde URL", key="btn_url_pdf", use_container_width=True):
            if url_pdf.strip():
                try:
                    with st.spinner("Descargando PDF desde la URL..."):
                        response = requests.get(url_pdf.strip(), timeout=15)
                        if response.status_code == 200:
                            if extraer_texto_pdf_desde_bytes(response.content, "PDF_Remoto.pdf"):
                                if procesar_texto_para_remates(st.session_state.texto_completo_pdf):
                                    st.success("✅ ¡Inscritos descargados y procesados correctamente desde la URL!")
                                    st.rerun()
                        else:
                            st.error(f"❌ Error al descargar. Código HTTP: {response.status_code}")
                except Exception as e:
                    st.error(f"❌ Error de conexión con la URL: {e}")
            else:
                st.warning("⚠️ Ingresa una URL válida.")

        st.markdown("---")
        pdf_subido = st.file_uploader("O sube el programa oficial en PDF", type=["pdf"], key="adm_pdf_uploader")
        if pdf_subido is not None:
            if st.button("📥 Cargar PDF en Memoria", key="adm_pdf_btn_cargar", use_container_width=True):
                if extraer_texto_pdf_desde_bytes(pdf_subido.getvalue(), pdf_subido.name):
