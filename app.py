        ...
        if not lista_carreras_disponibles:
            ...
            if not carreras_filtradas_visibles:
                ...
                if not remate_iniciado:
                    st.warning(f"⏳ **Remate no iniciado:** Esta carrera abrirá automáticamente el **{dt_inicio_remate.strftime('%d/%m/%Y a las %I:%M %p')}**.")
                else:
                    ...
                    # --- POTE, PREMIO TOTAL LLAMATIVO E INCENTIVO SENCILLO DEBAJO DE LA TABLA ---
                    ...
                    # --- PREMIO TOTAL COMO BLOQUE LLAMATIVO ---
                    st.markdown(f"""
                        <div class="incentivo-llamativo">
                            <div style="font-size: 13px; color: #00ffff; font-weight: 800; margin-bottom: 2px;">🏆 PREMIO TOTAL</div>
                            <div class="incentivo-llamativo-monto">{formatear_bs(premio_total_calculado)}</div>
                        </div>
                    """, unsafe_allow_html=True)

                    # --- POTE E INCENTIVO SENCILLO ABAJO ---
                    c_m1, c_m2 = st.columns(2)
                    c_m1.metric(f"💰 Pote ({carr_activa})", formatear_bs(total_pote))
                    c_m2.metric(f"🎁 Incentivo", formatear_bs(incentivo_actual))
