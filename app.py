...
import streamlit.components.import streamlit as st
...
# --- BOTONES SUPERIORES ESTILO TARJETA PARA NAVEGACIÓN ---
col_nav1, col_nav2, col_nav3, col_nav4 = st.columns([3, 3, 3, 2])
            ...
            with col_ps2:
                if st.button("⚡ Procesar PDF Completo Organizado", use_container_width=True):
                    if procesar_texto_para_remates(st.session_state.texto_completo_pdf):
                        ...
                        st.error("⚠️ No se pudo procesar automáticamente el documento.")v1 as components
import pandas as pd
import os
import re
import base64
...
# --- CABECERA PERSONALIZADA ESTILO REFERENCIA (CON MENÚ, LOGO, BANCO Y PERFIL) ---
st.markdown(f"""
    <style>
    .header-container {{
        background-color: #000000 !important;
    ...
    <div class="nav-sections-bar">
        <div class="nav-section-item">Remates</div>
        <div class="nav-section-item">Dupletas</div>
        <div class="nav-section-item">Cuentas</div>
    </div>
""", unsafe_allow_html=True)
...
# --- NAVEGACIÓN POR PESTAÑAS (CONTROLADAS MEDIANTE RADIO BUTTONS EN LA ZONA SUPERIOR) ---
menu_principal_opcion = st.radio(
    "Menú Principal",
    ["Remates", "Dupletas", "Cuentas", "🔒 Zona Admin"],
    horizontal=True,
